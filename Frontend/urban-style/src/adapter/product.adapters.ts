...
export class ProductAdapter {
	private static async formDataToProduct(formData: FormData): Promise<Products> {
		const { id, name, description, categories, price, attributes, discount, images } =
	...
	static async formDataToCreateProduct(formData: FormData): Promise<CreateProduct> {
		const { id, stock, ...createProduct } = await this.formDataToProduct(formData)
	...
	static async formDataToUpdateProduct(formData: FormData, id: string): Promise<UpdateProduct> {
		const { stock, images, ...updateProduct } = await this.formDataToProduct(formData)
