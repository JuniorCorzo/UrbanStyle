...
export function useAttributes(defaultAttributes: AttributesWithId[]) {
	...
	const handleChangeQuantity = (id: string, quantity: number) => {
		...
		attributeStore.set(attributes.map((attr) => (attr.id === id ? { ...attr, quantity } : attr)))
	}

	const getAttributesInputs = ($container: HTMLDivElement) => {
		const $colorCombobox = $container.querySelector<HTMLInputElement>("input[name='color']")
		const $sizeSelect = $container.querySelector<HTMLInputElement>("input[name='size']")
	...
	const handleAddAttribute = () => {
		...
		const { $colorCombobox, $quantityInput, $sizeSelect } = getAttributesInputs($container)
		...
		const isDuplicate = attributes.some(
			({ color, size }) => size === insertAttribute.size && color === insertAttribute.color,
		)
