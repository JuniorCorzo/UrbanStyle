...
export function ProductVariantSelectors({
  ...
  const colorOptions = useMemo(
    () =>
      Array.from(new Set(attributes.map((attr) => attr.color))).map(
  ...
  return (
    <div className="w-full min-h-[42px] flex flex-col sm:flex-row gap-4">
      <div className="grow">
        <SelectInput
          label="Color:"
          name="color"
          ...
          value={
            defaultColor
              ? { text: defaultColor, value: defaultColor }
              : colorOptions[0]
          }
      ...
      <div className="basis-1/3" key={sizeOptions?.length}>
        <SelectInput
          ...
          options={sizeOptions}
          value={
            defaultSize ? { text: defaultSize, value: defaultSize } : undefined
          }
