...
export function useImages(defaultImages: Images[]) {
	...
	useLayoutEffect(() => {
		...
		const imagesState = defaultImages?.map(async ({ image, color }): Promise<ImageState> => {
			...
			const imageName =
				productStore.get().find(({ images }) => images.find((img) => img.image === image))?.name ??
				''
	...
	const handleChangeColor = (imageUrl: string, color: string) => {
		imagesStore.set(
			images.map((image) => (image.imageUrl === imageUrl ? { ...image, color } : image)),
		)
	}
