...
public record ProductSummary(
                @NotBlank(groups = {
                                OnCreate.class, OnUpdate.class }) String productId,
                @NotBlank(groups = { OnCreate.class, OnUpdate.class }) String name,
                @DecimalMin(value = "0.0", message = "Price must be greater than or equal to zero", groups = {
                                OnCreate.class, OnUpdate.class }) double price,
                @Min(value = 0, message = "Quantity must be greater than or equal to zero", groups = { OnCreate.class,
                                OnUpdate.class }) int quantity,
                @Min(value = 0, message = "Discount must be greater than or equal to zero", groups = { OnCreate.class,
